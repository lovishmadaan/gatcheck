...
do
    ...
    do
        for approximate in -1 2
        do
            for attention_heads in 1 4 6
            do
                for out_attention_heads in 1 4 6
                do
                    for activation in 'LeakyRelu' 'tanh' 'relu'
                    do
                        ...
                        echo "${a}"
                        time python -u main.py --task link --dataset ${dataset} --model GAT --activation ${activation} --gpu --cuda 4 \
