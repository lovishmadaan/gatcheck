...
class GAT(torch.nn.Module):
    def __init__(self, input_dim, attention_heads, out_attention_heads, activation, feature_dim, hidden_dim, output_dim,
        ...
        if feature_pre:
            self.linear_pre = nn.Linear(input_dim, feature_dim)
            self.conv_first = tg.nn.GATConv(feature_dim, hidden_dim, heads=attention_heads)
        else:
...
class GAT_R(torch.nn.Module):
    def __init__(self, input_dim, attention_heads, out_attention_heads, activation, feature_dim, hidden_dim, output_dim,
        ...
        if feature_pre:
            self.linear_pre = nn.Linear(input_dim, feature_dim)
            self.conv_first = GATConv_Modified(feature_dim, hidden_dim, heads=attention_heads)
        else:
            self.conv_first = GATConv_Modified(input_dim, hidden_dim, heads=attention_heads)
